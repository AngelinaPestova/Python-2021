...
def make_block_gradation(start_y, start_x, arr, block_sum, block_size, step, gradations_amount):
    gradation_size = step * gradations_amount
    for row in range(start_y, start_y + block_size):
        for col in range(start_x, start_x + block_size):
            arr[row][col][0] = int(block_sum // gradation_size) * gradation_size
            arr[row][col][1] = int(block_sum // gradation_size) * gradation_size
            arr[row][col][2] = int(block_sum // gradation_size) * gradation_size
...
def main():
    ...
    parser.add_argument("-gradation_amount", type=int, default=1)
    ...
    for y in range(0, height - 1, block_size):
        for x in range(0, width - 1, block_size):
            ...
            make_block_gradation(y, x, arr, block_sum, block_size, step, gradation_amount)
